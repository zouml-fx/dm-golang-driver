...
func newOffRowBytesBinder(obj []byte, encoding string) *offRowBytesBinder {
	...
	binder.offRow = binder.buffer.Dm_build_1209() > Dm_build_397
	return binder
}

func (b *offRowBytesBinder) read(buf *Dm_build_1204) {
	if b.buffer.Dm_build_1209() > 0 {
		buf.Dm_build_1241(b.buffer)
	} else if !b.readOver {
		var obj = b.obj.([]byte)
		buf.Dm_build_1230(obj, 0, len(obj))
		b.readOver = true
	}
}
...
func (b *offRowBlobBinder) read(buf *Dm_build_1204) {
	if b.buffer.Dm_build_1209() > 0 {
		buf.Dm_build_1241(b.buffer)
	} else if !b.readOver {
		...
		buf.Dm_build_1230(bytes, 0, len(bytes))
	}
}
...
func (b *offRowClobBinder) read(buf *Dm_build_1204) {
	if b.buffer.Dm_build_1209() > 0 {
		buf.Dm_build_1241(b.buffer)
	} else if !b.readOver {
		...
		var bytes = Dm_build_923.Dm_build_1139(str, b.encoding, nil)
		b.position += readLen
		if b.position == int32(totalLen) {
			b.readOver = true
		}
...
func (b *offRowReaderBinder) read(buf *Dm_build_1204) {
	if b.buffer.Dm_build_1209() > 0 {
		buf.Dm_build_1241(b.buffer)
	} else if !b.readOver {
		var err error
		...
		buf.Dm_build_1230(bytes[0:readLen], 0, readLen)
	}
}

func (b *offRowReaderBinder) readAll() []byte {
	var byteArray = Dm_build_1208()
	b.read(byteArray)
	for !b.readOver {
		b.read(byteArray)
	}
	return byteArray.Dm_build_1251()
}
